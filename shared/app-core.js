...
const routeConfig = window.STUDY_ROUTE_CONFIG || {};
const LANGUAGE_STORAGE_KEY = "german-study-language";
const HOME_PAGE_PATH = String(routeConfig.homePath || "index.html");
const LIST_PAGE_PATH = String(routeConfig.listPath || "grammar/list.html");
const DETAIL_PAGE_PATH = String(routeConfig.detailPath || "grammar/detail.html");
const outlineUi = {
  zh: {
...
const FIXED_A1_LIST_PART = A1_LIST_PARTS.includes(String(routeConfig.fixedPart || "").toLowerCase())
  ? String(routeConfig.fixedPart).toLowerCase()
  : "";
const DEFAULT_A1_LIST_PART = FIXED_A1_LIST_PART || "grammar";
...
function buildTopicUrl(level, topicId, part = "grammar") {
  ...
  const partQuery = normalizedPart !== "grammar"
    ...
    : "";
  return `${DETAIL_PAGE_PATH}?level=${encodeURIComponent(level)}&topic=${encodeURIComponent(topicId)}${partQuery}`;
}

function buildSubunitUrl(level, topicId, subunitCode) {
  return `${DETAIL_PAGE_PATH}?level=${encodeURIComponent(level)}&topic=${encodeURIComponent(topicId)}&subunit=${encodeURIComponent(subunitCode)}`;
}
...
function renderListPage() {
  ...
  const homeLink = document.getElementById("list-home-link");
  ...
  const urlPart = FIXED_A1_LIST_PART || normalizeA1ListPart(params.get("part"));
  const activePart = FIXED_A1_LIST_PART
    || setA1ListPart(
  ...
  if (!hasGroupedOutline) {
    ...
    list.hidden = false;
    if (activePart === "reading" && Array.isArray(readingCurrent.items) && readingCurrent.items.length > 0) {
      setText("list-section-title", currentLang === "en" ? "Reading Sets" : "阅读题组");
      description.textContent = getLevelDescription(level, "reading");
...
function renderReadingTopicPage(level, topic, elements, sourcePart = "reading") {
  ...
  if (backLink) {
    backLink.href = `${LIST_PAGE_PATH}?level=${encodeURIComponent(level)}&part=${encodeURIComponent(sourcePart)}`;
    backLink.textContent = currentLang === "en" ? `Back to ${level} Reading` : `返回${level}阅读`;
  }

  if (breadcrumb) {
    breadcrumb.href = HOME_PAGE_PATH;
...
function renderDetailPage() {
  ...
  const part = FIXED_A1_LIST_PART || normalizeA1ListPart(params.get("part")) || DEFAULT_A1_LIST_PART;
  ...
  if (backLink) {
    ...
    backLink.href = `${LIST_PAGE_PATH}?level=${level}${partQuery}`;
